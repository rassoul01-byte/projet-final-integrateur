"""
import_csv.py — Import des données depuis donnees_propres.csv vers PostgreSQL
...
def get_or_create_classe(cursor, nom_classe: str) -> int:
...
def etudiant_existe(cursor, numero: str) -> bool:
    cursor.execute("SELECT 1 FROM etudiants WHERE numero = %s", (numero,))
    return cursor.fetchone() is not None


def inserer_etudiant(cursor, row: dict) -> int:
    ...
    try:
        with open(CSV_PATH, newline="", encoding="utf-8") as f:
            ...
            for ligne, row in enumerate(reader, start=2):
                ...
                if not numero:
                    print(f"  [SKIP] Ligne {ligne} : numéro manquant")
                    ignores += 1
                    continue
